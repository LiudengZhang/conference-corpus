...
4. **The contrarian voice has arrived (Feb 2026).** [Foundation Models Improve Perturbation Response Prediction](https://www.biorxiv.org/content/10.64898/2026.02.18.706454v1) (bioRxiv 2026.02.18) claims that with **sufficient data**, FMs do significantly improve genetic + chemical perturbation predictions and approach fundamental performance limits. The reckoning is not yet a one-sided consensus. The honest framing: 2024–2025 showed *current scFMs trained on current data with current evals* fail to clear linear baselines; whether *the FM paradigm* fails is a separate, still-open question — and theoretical work like *[Virtual Cells Need Context, Not Just Scale](https://www.biorxiv.org/content/10.64898/2026.02.04.703804v1)* (bioRxiv 2026.02.04) argues it's a **causal transportability problem**, not a model-capacity problem.
...
- **[CZ Biohub + CZI](https://chanzuckerberg.com/science/programs-resources/virtual-cells-initiative/)** — Chan Zuckerberg Initiative (funder) + the CZ Biohub network (SF / Chicago / NY). [Theofanis Karaletsos](https://tech.chanzuckerberg.com/ai-powering-biomedical-science/) heads AI for Science; Steve Quake was ex-Head of Biohub SF; Angela Pisco leads single-cell. Funds + ships: CELLxGENE Census (the canonical sc-FM training corpus), Tabula Sapiens, [the Virtual Cell Platform](https://virtualcellmodels.cziscience.com/), [TranscriptFormer](https://github.com/czi-ai/transcriptformer) (*Science* 2025 — first generative cross-species sc-FM; 112M cells × 12 species × 1.53B years of evolution), and [rBio](https://github.com/czi-ai/rbio) (Qwen-2.5-3B post-trained with GRPO using TranscriptFormer as a soft-supervision verifier — the first reasoning model grounded in virtual-cell models). **Strategic posture (revised 2025–2026)**: started as substrate-only; TranscriptFormer + rBio mean CZ Biohub now ships *both* substrate and model. The §3.11 "substrate vs model split" observation is partially out of date — fold the correction in when re-delivering.
...
The original bibliography in §11.3 leaned 2024–2025. Below are **37 papers and models** from mid-2025 through Q2 2026 that materially affect the talk's argument. Each is annotated with **what changed because of it** and **which section it should be folded into when re-delivering**.
...
- **[Liu et al. 2026 *Advanced Science*](https://advanced.onlinelibrary.wiley.com/doi/10.1002/advs.202514490)** — "Evaluating the Utilities of Foundation Models in Single‐Cell Data Analysis." `scEval` framework — 10 scFMs across 8 tasks. Headline: *"single-cell foundation models may not consistently outperform task-specific methods in all tasks, which challenges the necessity of developing foundation models for single-cell analysis."* **What changed**: broadens the linear-baseline critique past perturbation-only — the strongest "is the FM paradigm worth it?" framing to date. **Fold into**: §4.1 (the reckoning is no longer perturbation-only), §1.3 (the thesis line).
- **[Wu et al. 2025 *Genome Biology*](https://pmc.ncbi.nlm.nih.gov/articles/PMC12492631/)** — "Biology-driven insights into the power of single-cell foundation models" (Oct 2025). 6 scFMs (Geneformer, scGPT, UCE, scFoundation, LangCell, scCello) on gene- + cell-level tasks with cell-ontology-grounded metrics. Headline: *"no single scFM consistently outperforms others across all tasks."* (Distinct from Wu et al. 2025 *Nat Methods* above.) **Fold into**: §4.1, §5.5.
- **[Parameter-free representations outperform single-cell foundation models on downstream benchmarks, bioRxiv 2026.02.11.705358](https://www.biorxiv.org/content/10.64898/2026.02.11.705358v1)** (Feb 2026) — direct successor to Ahlmann-Eltze; the cleanest post-reckoning headline. **Fold into**: §4.1.
- **[PertEval-scFM, ICML 2025](https://icml.cc/virtual/2025/poster/43799)** — formal venue acceptance of the standardized framework for evaluating scFM embeddings on perturbation effect prediction. *"Most models do not outperform simple baselines, particularly when evaluated on strong or atypical perturbations."* **Fold into**: §4.1 (companion to Ahlmann-Eltze).
- **[CellBench-LS, bioRxiv 2026.04.01.714123](https://www.biorxiv.org/content/10.64898/2026.04.01.714123v1)** (April 2026) — stratified low-supervision protocol; 7 scFMs (scGPT, Geneformer, LangCell, CellPLM, scMulan, scFoundation, Nicheformer) + PCA/UMAP/scVI baselines. Foundation models lead on cell-type recognition; classical methods stay competitive on gene-expression quantification. **Fold into**: §4.1, §5.5.
- **[Han et al. — Benchmarking scFMs for real-world RNA-seq data integration, bioRxiv 2026.04.17.719314](https://www.biorxiv.org/content/10.64898/2026.04.17.719314v1)** (April 2026; Mansi et al., industry-authored) — pharma-relevant deployment-grade evaluation of scFM robustness. **Fold into**: §4.1, §8 (commercial signals).
- **[Benchmarking zero-shot scFM embeddings for cellular dynamics reconstruction, bioRxiv 2026.03.10.710748](https://www.biorxiv.org/content/10.64898/2026.03.10.710748v1)** (March 2026) — extends the critique to RNA-velocity / dynamics, a new axis beyond perturbation. **Fold into**: §4.1, §5.5.
...
- **[Foundation Models Improve Perturbation Response Prediction, bioRxiv 2026.02.18.706454](https://www.biorxiv.org/content/10.64898/2026.02.18.706454v1)** (Feb 2026) — **the contrarian voice**. Claims that with sufficient data, FMs *do* significantly improve genetic + chemical perturbation predictions and approach fundamental performance limits — directly opposing Ahlmann-Eltze, Liu, parameter-free, and the Wu papers. **What changed**: the §4 reckoning is no longer one-sided; honesty requires citing this even if you ultimately disagree. **Fold into**: §4.3 (the contrarian footnote, now added).
- **[MAP — Knowledge-driven framework for unprofiled drugs, bioRxiv 2026.02.25.708091](https://www.biorxiv.org/content/10.64898/2026.02.25.708091v1)** (Feb 2026) — integrates structured biological knowledge into perturbation modeling; supports zero-shot prediction for small molecules with no profiles. **What changed**: a credible Lane 3 / Track 4 candidate that adds biology-specific inductive bias rather than scaling. **Fold into**: §6.4, §7.5.
...
- **[Theis et al. 2026 *Cell Systems* Perspective](https://www.cell.com/cell-systems/abstract/S2405-4712(26)00016-5)** — "From modality-specific to compositional foundation models for cell biology" (Feb 2026). The post-reckoning reframing from the field's most credible critic lab: stop training bigger monolithic scFMs, start composing modality-specific FMs. **What changed**: pairs with the Bunne *Cell* "Turing test" paper as the two canonical "what comes next" references. **Fold into**: §4.3 (what it means going forward), §10 (closing).
- **[Virtual Cells Need Context, Not Just Scale, bioRxiv 2026.02.04.703804](https://www.biorxiv.org/content/10.64898/2026.02.04.703804v1)** (Feb 2026) — uses **causal-inference transportability theory** to argue the field has a *causal transport problem* that cannot be solved by accumulating more same-distribution data. Demonstrated on a 22M-cell immunology SOTA model. **What changed**: gives the §4 reckoning its theoretical underpinning — the failure is structural (cross-context generalization), not just empirical. The single paper that makes §4.3 consequence #2 ("what scale wins is open again") rigorous. **Fold into**: §4.3, §5.3 (causal vs correlational gap).
- **[Beyond alignment: synergistic integration is required for multimodal cell foundation models, bioRxiv 2026.02.23.707420](https://www.biorxiv.org/content/10.64898/2026.02.23.707420v3)** (Microsoft, Feb 2026) — introduces **Synergistic Information Score (SIS)** based on partial-information-decomposition theory to evaluate multimodal compositional FMs. Tasks dominated by linear redundancies need only unimodal baselines; complex niche definitions benefit from synergy-aware integration. **What changed**: pairs with Theis *Cell Systems* — Theis names compositional FMs as the architectural answer; this paper provides the evaluation metric. **Fold into**: §5.5 (evaluation honesty), §7.4 (compositional generalization benchmarks).
- **[Cao, Lu & Qiu — Towards predictive virtual embryos with genomics and AI, *Nature Methods* 2026](https://www.nature.com/articles/s41592-026-03055-4)** (Mar 26, 2026) — extends the virtual-X agenda from cell → **embryo**: integrate single-cell + spatial data with AI to model mammalian embryogenesis across scales. **What changed**: widens the talk's scope. Embryo-scale "virtual X" has different evaluation constraints than cell-scale. **Fold into**: §1.1 (what is a virtual cell — now also embryo), §10 (closing).
...
- **[Pearce et al. 2025 *Science*](https://www.science.org/doi/10.1126/science.aec8514)** — **TranscriptFormer** (CZ Biohub / CZI, [GitHub](https://github.com/czi-ai/transcriptformer) · [bioRxiv 2025.04.25.650731](https://www.biorxiv.org/content/10.1101/2025.04.25.650731v1)) — the first **generative cross-species** sc-FM; 112M cells × 12 species × 1.53 billion years of evolution. SOTA on cross-species cell-type classification (incl. species separated by 685M years) and zero-shot disease-state identification in human cells. Open weights, public. **What changed**: this is the model that turns CZ Biohub from a substrate-only lab into a model-shipping lab — directly contradicts the §3.11 "substrate vs model split" observation and re-enters the post-2025 SOTA debate from a cross-species angle the other major scFMs (scGPT / Geneformer / scFoundation) cannot match. **Fold into**: §2.1 (anchor list), §3 (an 11th deep-dive candidate), §3.11 (revise observation #2), §11.4 CZ Biohub dossier (already updated).
- **[xVERSE (Jiang & Xie, bioRxiv 2026.04.12.718016)](https://www.biorxiv.org/content/10.64898/2026.04.12.718016v1)** (April 2026) — **transcriptomics-native** FM coupling batch-invariant representation learning with probabilistic generation of expression profiles. Beats leading scFMs by **17.9% on representation, 11.4% on batch correction, 34.3% on spatial imputation**; synthesizes virtual cells indistinguishable from real (AUROC ≈ 0.5); resolves rare cell types with as few as 4 cells. **What changed**: a direct architectural shot at the "scGPT / Geneformer language-model architectures ignore transcriptomic data distributions" critique. If xVERSE replicates, the post-2025 SOTA debate gains an architectural fork: language-model-derived vs transcriptomics-native. **Fold into**: §2.1 (anchor list), §4.3 (architectural counter-evidence), §7.5 (Track 4 — biology-specific inductive bias).
- **[OmniCell (Pang et al., bioRxiv 2025.12.29.696804)](https://www.biorxiv.org/content/10.64898/2025.12.29.696804v1)** (Dec 2025) — first FM to **jointly capture intra-cellular gene-expression + inter-cellular spatial dependencies** in a unified framework; 67M single-cell + spatial profiles. Serializes spatially adjacent cells during input construction. **What changed**: a competitor to Nicheformer with a different inductive bias (spatial topology as token sequence, not as auxiliary modality). **Fold into**: §2.1 (spatial line), §3.11 (Helmholtz Munich is no longer the only spatial-sc-FM lab).
- **[VCHarness (Cheng et al. / Le Song / Eric Xing, bioRxiv 2026.04.11.717183)](https://www.biorxiv.org/content/10.64898/2026.04.11.717183v1)** (April 2026, BioMap Research + MBZUAI) — **autonomous AI system** that constructs virtual-cell perturbation-response models from scratch, combining an AI coding agent with multimodal biological FMs. Identifies architectures that outperform expert designs; reduces development time from months to days. **What changed**: the agentic ↔ virtual-cell convergence acquires a *builder* (vs rBio's *reasoner*). Pairs with rBio: rBio reasons over an existing virtual cell, VCHarness builds new ones. **Fold into**: §1.2 (Why now), §7.5 (architectures search), §8 (commercial signal — BioMap is the China-side competitor to Owkin / Modella).
- **[SpatialFusion, bioRxiv 2026.03.16.712056](https://www.biorxiv.org/content/10.64898/2026.03.16.712056v1)** (Mar 2026) — lightweight multimodal FM for pathway-informed spatial niche mapping. **What changed**: completes the §2.1 spatial FM line (Nicheformer + OmniCell + SpatialFusion now form a class). **Fold into**: §2.1.
- **[rBio v1, CZ Biohub 2025](https://virtualcellmodels.cziscience.com/model/rbio)** — Qwen2.5-3B-Instruct post-trained via GRPO using TranscriptFormer as a soft-supervision verifier. First **reasoning model trained on virtual-cell simulations** — answers natural-language questions about gene interactions ("Would suppressing gene A increase activity of gene B?") with biological grounding from a sc-FM rather than from web text. **What changed**: the agentic-AI ↔ virtual-cell convergence finally has a public model; previously this was vapor. **Fold into**: §1.2 (Why now), §7.2 (interpretability — rBio is the inverse direction), §11.4.
...
- **[CLM-X, bioRxiv 2026.02.17.704943](https://www.biorxiv.org/content/10.64898/2026.02.17.704943v1)** (Feb 2026) — multimodal sc-FM with flexible multi-way Transformer for unified scRNA-seq + scATAC-seq; outperforms unimodal scFMs on RNA↔ATAC cross-modal translation and genetic-perturbation-response prediction. **Fold into**: §2.1 (multimodal line), §5.1.
- **[CytoVerse, bioRxiv 2026.01.29.702554](https://www.biorxiv.org/content/10.64898/2026.01.29.702554v1)** (Jan 2026) — browser-native scFM inference via ONNX; serves a >20M-cell reference from client-side. **What changed**: the *deployment* axis the talk under-covers — scFMs as embedded infrastructure rather than as research artifacts. **Fold into**: §6.7 (Lane 6 — FM-wrapper tools).
- **[CELLama (Park et al. 2026 *Adv Sci*)](https://advanced.onlinelibrary.wiley.com/doi/10.1002/advs.202513210)** — language-model-embedding–based FM for single-cell + spatial transcriptomics; treats cells as natural-language descriptions. **Fold into**: §2.1, §2.5 (multimodal bridge).
...
- **[Yan et al. 2025 *npj Digital Medicine* — PathOrchestra](https://www.nature.com/articles/s41746-025-02027-w)** (Nov 2025) — versatile pathology FM trained on 287,424 slides across 21 tissue types from three centers; evaluated on 112 tasks (digital slide preprocessing, pan-cancer classification, lesion ID, biomarker assessment, gene-expression prediction, structured report generation). **What changed**: extends the 2025–2026 pathology-FM consolidation story; the 112-task evaluation suite is itself a contribution. **Fold into**: §2.2 anchor list, §6.7 (FM-wrapper tools).
...
- **§1.3 thesis line gets sharper.** Liu 2026 *Adv Sci* + Theis 2026 *Cell Systems* generalize the linear-baseline reckoning past perturbation-only — the question is no longer "do scFMs predict perturbations" but "does the monolithic-FM paradigm earn its compute at all, vs. composing modality-specific FMs."
- **§3.11 substrate-vs-model observation needs a footnote.** TranscriptFormer (CZ Biohub, *Science* 2025) and rBio (CZ Biohub, 2025) flip CZ Biohub from substrate-only to *both* — re-read the §3.11 observation #2 as "the substrate-vs-model split *was* the field's structural feature, until CZ Biohub bridged it in 2025."
- **§4.1 evidence base is now ten papers, not three.** Ahlmann-Eltze, Kedzierska, Wenkel, Wu *Nat Methods*, Wu *Genome Biol*, Liu *Adv Sci*, parameter-free bioRxiv, PertEval-scFM, CellBench-LS, Han et al., cellular-dynamics zero-shot. The reckoning has moved from "three independent papers" to "a discipline-wide consensus that single-cell FMs don't yet earn the linear-baseline floor across most downstream tasks."
- **§4.3 gains a contrarian footnote** (now folded in): *Foundation Models Improve Perturbation Response Prediction* (bioRxiv 2026.02.18) argues that with sufficient data, FMs *do* improve perturbation predictions. The reckoning is no longer one-sided; honest framing requires citing this even if you ultimately disagree.
- **§4 also gains a theoretical underpinning.** *Virtual Cells Need Context, Not Just Scale* (bioRxiv 2026.02.04) frames the failure as a **causal transportability problem** — structural, not capacity-bounded. This is the paper §4.3 consequence #2 needed.
- **§5.3 (causal vs correlational gap) gets sharper.** *Virtual Cells Need Context* + *Beyond alignment (SIS)* now provide both the diagnosis (causal transport problem) and the evaluation metric (synergistic information score) for the causal-perturbation gap.
- **§7.5 Track 4 (architectures with biology-specific inductive bias) acquires real exemplars.** xVERSE (transcriptomics-native) and MAP (knowledge-driven for unprofiled drugs) — neither is a language-model-derived FM. The "all sc-FMs are scGPT-shaped" frame is out of date.
- **§6.8 Lane 7 gets a real clinical exemplar.** Janowczyk *Nat Med* 2025 (UNI for EGFR detection, 197 patients, 43% genetic-testing avoidance).
- **§7.2 Track 1 has to acknowledge the SAE wave broke** — but the *cancer-specific* SAE pitch is intact and arguably better.
- **§10 closing now has three reframing references.** Bunne *Cell* (Turing test bar), Theis *Cell Systems* (compositional FMs as the architectural answer), and Cao/Lu/Qiu *Nature Methods* 2026 (virtual *embryos* — the next scale up). The "virtual cell" agenda is now visibly forking into the cell-scale and embryo-scale tracks.
- **Pathology FM proliferation (TITAN, mSTAR, GPFM, PathPT, PathOrchestra)** is the strongest evidence that the field is leaning into Lane 7 (clinical deployment) over Lane 3 (more FMs).
- **Agentic ↔ virtual-cell convergence is now bidirectional**: rBio *reasons* over a virtual cell (TranscriptFormer as verifier); VCHarness *builds* virtual cells (AI coding agent + multimodal FMs). The §8 commercial framing should note that CZ Biohub (TranscriptFormer + rBio) and BioMap (VCHarness) are the two fully-open / open-research stacks vs the closed commercial agents from Owkin / Modella AI.
...
*Last updated: 2026-05-13. Resource numbers and field status reflect the May 2026 state. §11.6 captures the 2025–2026 papers (the broadened evaluation-paper wave — Liu 2026 *Adv Sci*, Wu 2025 *Genome Biol*, parameter-free bioRxiv, PertEval-scFM, CellBench-LS, Han et al., cellular-dynamics zero-shot; the contrarian voice — Foundation Models Improve Perturbation Response Prediction bioRxiv 2026.02; the theoretical underpinning — Virtual Cells Need Context bioRxiv 2026.02 + Beyond Alignment / SIS Microsoft bioRxiv 2026.02; Bunne 2025 *Cell* "Turing test"; Theis 2026 *Cell Systems* compositional-FM Perspective; Cao/Lu/Qiu *Nature Methods* 2026 predictive virtual embryos; Janowczyk 2025 *Nat Med* clinical UNI deployment; the 5-paper SAE-on-biology-FMs wave; TITAN / mSTAR / GPFM / PathPT / PathOrchestra pathology consolidation; Nicheformer / CellFM / CLM-X / CytoVerse / CELLama / xVERSE / OmniCell / VCHarness / SpatialFusion / MAP new sc-FMs; TranscriptFormer + rBio bridging CZ Biohub from substrate to model; AlphaGenome critique in *Cell Research*) that should be folded into the body when re-delivering.*
